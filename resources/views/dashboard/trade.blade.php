   ...
   <x-slot name="header">
    ...
    <!-- Link -->
        <div class="flex justify-between p-4">
            <a  href="{{ route('create.title') }}" type="button" class="inline-block px-6 py-2.5 bg-purple-900 text-white font-medium text-xs leading-tight rounded shadow-sm hover:bg-purple-700 hover:shadow-lg
                            ...
                            <td class="text-sm text-gray-900 px-6 py-4 whitespace-nowrap">
                                <div x-data="{ show: false }"  @click.away="show = false" class="relative inline-block text-left mb-2">
                                    ...
                                    <div x-show="show" class="absolute bg-white z-10 shadow-md w-40">
                                        <a class="block px-3 py-2" href="{{ route('show.invoice',['id' => $trade->id]) }}">Invoice</a>
                                        @if ($trade->type == 'Selling')
                                        <a class="block px-3 py-2" href="{{ route('show.estimate',['id' => $trade->id]) }}">Estimate</a>
                                        @endif
                                    </div>
                                </div>
                            </td>
